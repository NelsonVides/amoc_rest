-module(amoc_rest_scenarios_handler_SUITE).
...
end_per_testcase(TestCase, _Config)
    ...
    amoc_rest_helper:remove_module(?SAMPLE_SCENARIO),
    amoc_rest_helper:stop_amoc();
end_per_testcase(_, _Config) ->
    amoc_rest_helper:stop_amoc().

get_scenarios_returns_200_and_scenarios_list_when_requested(_Config) ->
    %% when
    {CodeHttp, Body} = amoc_rest_helper:get(?SCENARIOS_URL_S),
    %% then
...
put_scenarios_returns_400_and_error_when_scenario_is_not_valid(_Config) ->
    ...
    %% when
    {CodeHttp, Body} = amoc_rest_helper:put(?SCENARIOS_URL_U, ScenarioContent),
    ScenarioFileSource = amoc_rest_helper:module_src(?SAMPLE_SCENARIO),
    %% then
...
put_scenarios_returns_200_when_scenario_valid(Config) ->
    ...
    {CodeHttp, Body} = amoc_rest_helper:put(?SCENARIOS_URL_U, ScenarioContent),
    ScenarioFileSource = amoc_rest_helper:module_src(?SAMPLE_SCENARIO),
    ScenarioFileBeam = amoc_rest_helper:module_beam(?SAMPLE_SCENARIO),
    %% then
    ?assertEqual(200, CodeHttp),
...
get_scenario_info_returns_404_when_scenario_does_not_exist(_Config) ->
    %% when
    {CodeHttp, _Body} = amoc_rest_helper:get(?SCENARIOS_URL_I(?SAMPLE_SCENARIO)),
    %% then
...
get_scenario_info_returns_200_when_scenario_exists(Config) ->
    ...
    %% when
    {CodeHttp, Body} = amoc_rest_helper:get(?SCENARIOS_URL_I(?SAMPLE_SCENARIO)),
    ?assertEqual(200, CodeHttp),
    ExpectedInfo = #{<<"doc">> => <<"\nsome edoc\n\n">>,
                     <<"parameters">> =>
                         #{<<"interarrival">> =>
...
get_scenario_defaults_returns_404_when_scenario_does_not_exist(_Config) ->
    %% when
    {CodeHttp, _Body} = amoc_rest_helper:get(?SCENARIOS_URL_D(?SAMPLE_SCENARIO)),
    %% then
    ?assertEqual(404, CodeHttp).
...
get_scenario_defaults_returns_200_when_scenario_exists(Config) ->
    ...
    mock_amoc_amoc_scenario(),
    %% when
    {CodeHttp, Body} = amoc_rest_helper:get(?SCENARIOS_URL_D(?SAMPLE_SCENARIO)),
    ?assertEqual(200, CodeHttp),
