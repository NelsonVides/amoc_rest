-module(amoc_rest_status_handler_SUITE).
...
returns_up_when_amoc_up_and_idle(Config) ->
    ...
    {CodeHttp, Body} = amoc_rest_helper:get(StatusPath),
    %% then
...
remote_node_status_returns_404_for_unavailable_node(_Config)->
    {CodeHttp, Body} = amoc_rest_helper:get("/status/unavailable@node"),
...
remote_node_status_returns_400_for_invalid_node(_Config)->
    ...
    {CodeHttp, Body} = amoc_rest_helper:get("/status/invalid_node_name"),
...
returns_nodes_list_when_amoc_up(_Config) ->
    ...
    %% when
    {CodeHttp, JSON} = amoc_rest_helper:get(?NODES_PATH),
    %% then
    ?assertEqual(200, CodeHttp),
