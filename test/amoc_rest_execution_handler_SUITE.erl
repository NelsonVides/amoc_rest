-module(amoc_rest_execution_handler_SUITE).
...
end_per_suite(_Config) ->
    amoc_rest_helper:remove_module(sample_test),
...
start_scenario(_Config) ->
    ...
    {HttpCode, Body} = amoc_rest_helper:patch("/execution/start", JsonBody),
...
stop_scenario(_Config) ->
    {HttpCode, Body} = amoc_rest_helper:patch("/execution/stop"),
    ?assertEqual(200, HttpCode),
    ?assertEqual(#{}, Body).
...
add_users(_Config) ->
    JsonBody = #{users => 10},
    {HttpCode, Body} = amoc_rest_helper:patch("/execution/add_users", JsonBody),
    ?assertEqual(200, HttpCode),
    ?assertEqual(#{}, Body).
...
remove_users(_Config) ->
    JsonBody = #{users => 10},
    {HttpCode, Body} = amoc_rest_helper:patch("/execution/remove_users", JsonBody),
    ?assertEqual(200, HttpCode),
    ?assertEqual(#{}, Body).
...
update_settings(_Config) ->
    ...
    {HttpCode, Body} = amoc_rest_helper:patch("/execution/update_settings", JsonBody),
    ?assertEqual(200, HttpCode),
    ?assertEqual(#{}, Body).
