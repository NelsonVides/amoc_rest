-module(amoc_rest_api_logic_handler).
...
handle_request('StatusGet', _Req, _Context) ->
    Status = amoc_rest_helpers_status:get_status(),
    {200, #{}, Status};
handle_request('StatusNodeGet', _Req, #{node := BinNode}) ->
    Node = binary_to_atom(BinNode, utf8),
    case rpc:call(Node, amoc_rest_helpers_status, get_status, []) of
        {badrpc, _} -> {404, #{}, #{}};
        Status -> {200, #{}, Status}
    end;
...
handle_request('ScenariosDefaultsIdGet', _Req, #{id := ScenarioName}) ->
    case amoc_rest_helpers_scenario_info:is_loaded(ScenarioName) of
        false ->
            {404, #{}, #{}};
        {true, Scenario} ->
            Settings = amoc_rest_helpers_scenario_info:scenario_settings(Scenario),
...
handle_request('ScenariosInfoIdGet', _Req, #{id := ScenarioName}) ->
    case amoc_rest_helpers_scenario_info:is_loaded(ScenarioName) of
        false ->
            {404, #{}, #{}};
        {true, Scenario} ->
            EDoc = amoc_rest_helpers_scenario_info:get_edoc(Scenario),
            Params = amoc_rest_helpers_scenario_info:scenario_params(Scenario),
            {200, #{}, #{doc => EDoc, parameters => Params}}
    end;
handle_request('ScenariosUploadPut', Req, _Context) ->
    {ok, ModuleSource, _} = cowboy_req:read_body(Req),
    case amoc_rest_helpers_scenario_upload:upload(ModuleSource) of
        ok ->
            {200, #{}, #{compile => <<"ok">>}};
...
handle_request('ExecutionStartPatch', _Req, #{'ExecutionStart' := Body}) ->
    case amoc_dist:get_state() of
        idle ->
            Ret = amoc_rest_helpers_execution:start(Body),
...
handle_request('ExecutionStopPatch', _Req, #{}) ->
    case amoc_dist:get_state() of
        running ->
            Ret = amoc_rest_helpers_execution:stop(),
            process_ret_value(Ret);
        _ -> {409, #{}, #{}}
    end;
handle_request('ExecutionAddUsersPatch', _Req, #{'ExecutionChangeUsers' := Body}) ->
    case amoc_dist:get_state() of
        running ->
            Ret = amoc_rest_helpers_execution:add_users(Body),
            process_ret_value(Ret);
        _ -> {409, #{}, #{}}
    end;
handle_request('ExecutionRemoveUsersPatch', _Req, #{'ExecutionChangeUsers' := Body}) ->
    case amoc_dist:get_state() of
        running ->
            Ret = amoc_rest_helpers_execution:remove_users(Body),
            process_ret_value(Ret);
        _ -> {409, #{}, #{}}
    end;
handle_request('ExecutionUpdateSettingsPatch', _Req, #{'ExecutionUpdateSettings' := Body}) ->
    case amoc_dist:get_state() of
        running ->
            Ret = amoc_rest_helpers_execution:update_settings(Body),
            process_ret_value(Ret);
        _ -> {409, #{}, #{}}
    end;
